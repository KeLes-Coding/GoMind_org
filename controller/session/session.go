...
type (
	...
	SessionTreeResponse struct {
		controller.Response
		Tree *model.SessionListTreeResponse `json:"tree,omitempty"`
	}
	...
	RemoveFromFolderRequest struct {
		SessionID string `json:"sessionId" binding:"required"`
	}
...
func GetSessionTree(c *gin.Context) {
	res := new(SessionTreeResponse)
	userID := c.GetInt64("userID")
	userName := c.GetString("userName")

	tree, code_ := session.GetSessionTree(userID, userName)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
...
func CreateFolder(c *gin.Context) {
	...
	_, code_ := session.CreateSessionFolder(userID, userName, req.Name)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
...
func RenameFolder(c *gin.Context) {
	...
	code_ := session.RenameSessionFolder(userID, req.FolderID, req.Name)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
...
func DeleteFolder(c *gin.Context) {
	...
	code_ := session.DeleteSessionFolder(userID, req.FolderID)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
...
func MoveSession(c *gin.Context) {
	...
	code_ := session.MoveSessionToFolder(userID, userName, req.SessionID, req.FolderID)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
...
func RemoveSessionFromFolder(c *gin.Context) {
	req := new(RemoveFromFolderRequest)
	res := new(controller.Response)
	userName := c.GetString("userName")
	if err := c.ShouldBindJSON(req); err != nil {
		c.JSON(http.StatusOK, res.CodeOf(code.CodeInvalidParams))
	...
	code_ := session.RemoveSessionFromFolder(userName, req.SessionID)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
...
func RenameSession(c *gin.Context) {
	...
	code_ := session.RenameSession(userName, req.SessionID, req.Title)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
