<template>
  <div class="min-h-screen flex flex-col items-center justify-center bg-bg-light dark:bg-bg-dark text-text-primary-light dark:text-text-primary-dark px-6">
    <div class="flex flex-col items-center gap-6 max-w-2xl w-full">
      <!-- Hero -->
      <div class="flex flex-col items-center gap-4">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-accent-light dark:text-accent-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>
        <h1 class="text-5xl font-bold tracking-tight select-none">GoMind</h1>
        <p class="text-lg text-text-secondary-light dark:text-text-secondary-dark text-center max-w-md">
          Powered by multi-provider AI · Ask anything
        </p>
      </div>

      <!-- CTA Buttons -->
      <div class="flex gap-4 mt-4">
        <router-link to="/ai-chat" class="px-8 py-3 rounded-full bg-black text-white dark:bg-white dark:text-black font-semibold text-base hover:opacity-90 transition-opacity">
          开始使用
        </router-link>
        <router-link to="/login" class="px-8 py-3 rounded-full border border-border-light dark:border-border-dark text-text-primary-light dark:text-text-primary-dark font-semibold text-base hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
          登录
        </router-link>
      </div>
      ...
      <div class="flex items-center gap-4 mt-10">
        ...
        <a href="#" title="WeChat" class="p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 text-text-secondary-light dark:text:text-text-secondary-dark hover:text-text-primary-light dark:hover:text-text-primary-dark transition-colors">
